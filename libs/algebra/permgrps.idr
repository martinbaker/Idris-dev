...
llc2lp : List (List (Cycle Nat)) -> List (Permutation Nat)
llc2lp llc = map cyclesToPermutation llc

--lllc2llp : (Eq s) => List (List (Cycle Nat)) -> List (Permutation Nat)
--lllc2llp lllc = map llc2lp lllc

lln2lc : List (List Nat) -> List (Cycle Nat) 
lln2lc lln = map cycle.fromList lln

llln2llc : List (List (List Nat)) -> List (List (Cycle Nat)) 
llln2llc llln = map lln2lc llln

llln2lp : List (List (List Nat)) -> List (Permutation Nat)
...
llli2gp : List (List (List Nat)) -> (PermutationGroup Nat)
...
dihedralGroup n =
  case n of
    (S Z) => llli2gp [[[1, 2]], [[3, 4]]] --n = 1 => symmetricGroup (2::PI)
...
main = 
  ...
  in
    do
      putStrLn ("permutation group c1=" ++ (show c1))
      putStrLn ("permutation group c2=" ++ (show c2))
      putStrLn ("permutation group c3=" ++ (show c3))
      putStrLn ("permutation group c4=" ++ (show c4))
      putStrLn ("permutation group d1=" ++ (show d1))
      putStrLn ("permutation group d2=" ++ (show d2))
      putStrLn ("permutation group d3=" ++ (show d3))
      putStrLn ("permutation group d4=" ++ (show d4))
